...
*                                registration.cpp
*该文件主要是简单的点云配准流程：
*预处理（去除NaN点、离群点滤除）、ISS关键点、FPFH描述子、SCIA、ICP
*输入：cloud_src.pcd源点云文件名 
*输出：cloud_tgt.pcd目标点云文件名
...
main(int argc, char** argv)
{
	...
	pcl::io::loadPCDFile("cloud_src.pcd", *cloud_src_o);
	PointCloud::Ptr cloud_tgt_o(new PointCloud);//目标点云
	pcl::io::loadPCDFile("cloud_tgt.pcd", *cloud_tgt_o);
	...
	voxel_grid.setLeafSize(0.012, 0.012, 0.012);//设置栅格化下采样的尺寸
